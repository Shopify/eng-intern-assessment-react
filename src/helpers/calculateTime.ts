...
const calculateTime = (timeInSeconds: number): TimeFormat => {
  ...
  const milliseconds =
    (timeInSeconds / millisecondsPerInterval) % maxMilliseconds;
  const minutes = Math.floor(
    (timeInSeconds / millisecondsInMinute) % secondsInMinute
  );
  const seconds = Math.floor(
    (timeInSeconds / millisecondsInSecond) % secondsInMinute
  );
